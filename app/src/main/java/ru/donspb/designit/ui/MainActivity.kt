...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TabLayoutMediator(tabLayout, viewPager) { tab, position ->
            when (position) {
                0 -> tab.text = getString(R.string.mi_home_name)
                1 -> tab.text = getString(R.string.mi_classes_name)
                2 -> tab.text = getString(R.string.mi_list_name)
                3 -> tab.text = getString(R.string.mi_fav_name)
            }
        }.attach()
    }
}